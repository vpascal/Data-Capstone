...
input <- function(x) {
  ...
  if (input_len == 1) {
    ...
    monogram <- paste(monogram, collapse = " ")
    bigram <-
      unlist(strsplit(x, " "))[(input_len - 2 + 1):input_len]
...
predictor <- function(phrase, bigram_table, trigram_table) {
  ...
  {
    temp1 <-
      str_detect(bigram_table$ngrams, paste0("^", paste0(phrase$monogram, " ")))
    ...
    if (nrow(temp1) == 0) {
      list(temp1 = "the",
           candidates = c("and", "for", "that", "with"))
    } else{
      temp1 <- temp1 %>% arrange(freq) %>% top_n(5)
      candidates <-
        temp1 %>% arrange(freq) %>% top_n(-4) %>% select(ngrams)
    ...
    temp2 <-
      str_detect(trigram_table$ngrams, paste0("^", paste0(phrase$bigram, " ")))
    ...
    if (nrow(temp2) > 0) {
      ...
      candidates <-
        temp2 %>% arrange(freq) %>% top_n(-4) %>% select(ngrams)
      candidates <- paste(unlist(candidates), sep = ",")
      ...
      candidates <-
        temp3 %>% arrange(freq) %>% top_n(-4) %>% select(ngrams)
      candidates <- paste(unlist(candidates), sep = ",")
...
box <- function(header, text, myicon) {
  HTML(
    paste0(
      "<div class='w3-quarter'> <div class='w3-card-2'><header class='w3-container w3-blue'> <h4>",
      header,
      "</h4></header><br>",
      myicon,
      "<p>",
